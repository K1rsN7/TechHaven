...
#dict_category = {'Компьютеры': 1, 'Процессоры': 2, 'Видеокарты': 3, 'Материнские платы': 4, 'Оперативная память': 5, 'Диски SSD': 6, 'Диски HDD': 7, 'Блоки питания': 8, 'Корпуса': 9, 'Кулеры': 10, 'Новинки': 11, 'Мониторы': 12, 'Аксессуары': 13, 'Клавиатуры и мыши': 14}
dict_category = {}
...
def generate_product_description(product_name):
    ...
    response = g4f.ChatCompletion.create(
        model=g4f.models.gpt_4o,  # Используем модель GPT-4
        messages=messages
    )
    return response
...
def data_processing_third_step():
    files = [file for file in os.listdir("./data") if file not in ['brand.json', 'category.json']]
    all_data = []
    for file in files:
        with open(f"./data/{file}", 'r', encoding='utf-8') as f:
            data = json.load(f)
    ...
    with open(f"./data/product.json", 'w', encoding='utf-8') as json_file:
